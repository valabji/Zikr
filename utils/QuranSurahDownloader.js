...
class QuranSurahDownloaderService {
  ...
  async start(reciterId, surah) {
    ...
    try {
      ...
      if (this.cancelled[k]) { this.state[k] = blankState(); this._emit(); NotificationService.hideDownloadProgress(this._notifId(reciterId, surah)); return; }
      ...
      if (this.cancelled[k]) {
        ...
        this._emit();
        NotificationService.hideDownloadProgress(this._notifId(reciterId, surah));
        return;
      }
