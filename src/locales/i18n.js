...
export const setLanguage = async (lang, restart = true) => {
  if (translations[lang]) {
    ...
    await AsyncStorage.setItem(APP_KEYS.LANGUAGE, lang);
...
export const toArabicDigits = (n) => String(n).replace(/\d/g, (d) => '٠١٢٣٤٥٦٧٨٩'[Number(d)]);
...
export const initializeLanguage = async () => {
  try {
    const savedLanguage = await AsyncStorage.getItem(APP_KEYS.LANGUAGE);
    if (savedLanguage) {
