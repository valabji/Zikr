...
import MainScreen from '@/screens/azkar/MainScreen';
...
function useInitialDrawerRoute() {
  ...
  useEffect(() => {
    AsyncStorage.getItem(APP_KEYS.FIRST_TIME_SETTINGS).then(firstTime => {
      ...
      AsyncStorage.getItem(APP_KEYS.INITIAL_SCREEN).then(screen => {
        const map = {
...
export function DrawerNavigation() {
  ...
  const loadMenuConfig = useCallback(async () => {
    ...
    const dateShown = await AsyncStorage.getItem(APP_KEYS.MENU_SHOW_DATE);
